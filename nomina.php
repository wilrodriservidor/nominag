...
try {
    ...
    // Mapeo de nombres por si las columnas se llaman diferente
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['empleado_id'])) {
    ...
    if ($contrato) {
        // CORRECCIÓN PARA EL ERROR 1054: Sumamos las columnas reales de tu DB
        $stmt_asistencia = $pdo->prepare("
            SELECT 
                SUM(horas_diurnas) as hd, 
                SUM(horas_nocturnas) as hn, 
                SUM(horas_festivas_diurnas + horas_festivas_nocturnas) as hf 
            ...
            <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden">
                        <div class="bg-slate-900 p-8 text-white flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-slate-400 text-xs font-bold uppercase mt-1">Resumen de Pago Quincenal</p>
                            </div>
                        ...
                        <div class="p-10">
                            <div class="grid grid-cols-2 gap-10">
                                <div class="space-y-6">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <span class="text-sm font-bold text-slate-600">Recargos Festivos</span>
                                        <span class="font-black text-slate-900">$<?= number_format($liquidacion['rf_val'], 0) ?></span>
                                    </div>
                            ...
                            <form action="guardar_nomina.php" method="POST" class="mt-12">
                                ...
                                <button type="submit" class="w-full bg-emerald-500 text-white py-5 rounded-2xl font-black text-xl shadow-xl hover:bg-emerald-600 transition flex items-center justify-center gap-3">
                ...
                <?php else: ?>
                    <div class="h-full flex flex-col items-center justify-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-20 text-center">
                        <i class="fas fa-calculator text-3xl text-slate-200 mb-6"></i>
