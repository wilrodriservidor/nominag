...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['empleado_id'])) {
    ...
    // Obtener contrato y verificar si es Dirección y Confianza
    ...
    if ($contrato) {
        ...
        $jornada = $ley['jornada_semanal_horas']; // Ej: 44
        ...
        // Aux Transporte: Solo si gana <= 2 SMMLV (Todo desde la BD)
        $v_aux_trans = ($salario_base <= ($ley['smmlv'] * 2)) ? proporcionarValor($ley['aux_transporte_ley'], $dias_periodo) : 0;

        // Auxilios del Contrato proporcionales a los días del periodo (15 días)
        ...
        // Seguridad Social con porcentajes de la BD (Ej: 0.04 para 4%)
        ...
        $liquidacion = [
            ...
            'es_confianza' => ($contrato['cargo'] == 'DIRECCION Y CONFIANZA' || $contrato['cargo'] == 'ADMINISTRADOR'),
...
<body class="bg-slate-100 min-h-screen">
    <div class="max-w-6xl mx-auto py-10 px-4">
        ...
        <header class="flex justify-between items-center mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
            <div>
                <h1 class="text-2xl font-black text-slate-800 italic">SYSTEM_NOMINA_2026</h1>
                <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Valores vinculados a Base de Datos</p>
            </div>
            <a href="index.php" class="text-slate-500 hover:text-indigo-600 font-bold text-sm transition"> <i class="fas fa-home mr-1"></i> Inicio</a>
        </header>
            ...
            <div class="lg:col-span-4">
                <div class="bg-white p-8 rounded-[2rem] shadow-xl border border-slate-200">
                    <form action="" method="POST" class="space-y-6">
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Seleccionar Personal</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Fecha Inicio</label>
                            ...
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase ml-2 mb-2 block">Fecha Fin</label>
                        ...
                        <button type="submit" class="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-indigo-700 shadow-xl transition transform active:scale-95 flex items-center justify-center gap-3">
                    ...
                    <div class="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden">
                        <div class="bg-slate-900 p-8 text-white">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h2 class="text-3xl font-black uppercase"><?= $liquidacion['nombre'] ?></h2>
                                    <?php if($liquidacion['es_confianza']): ?>
                                        <span class="text-amber-400 text-[10px] font-black border border-amber-400/30 px-2 py-1 rounded italic mt-2 inline-block">PERSONAL DE DIRECCIÓN Y CONFIANZA (SÓLO RECARGOS)</span>
                                    <?php endif; ?>
                                </div>
                                <div class="text-right">
                                    <p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Monto Neto</p>
                        ...
                        <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div class="space-y-4">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] border-b pb-3 mb-6">Conceptos de Pago</h3>
                                ...
                                <div class="flex justify-between items-center px-4">
                                    <span class="text-slate-400 text-sm font-medium">Recargos Totales</span>
                                ...
                                <div class="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 mt-4">
                                    <div class="flex justify-between text-xs text-indigo-700 font-bold">
                                        <span>Auxilio Movilización</span>
                            ...
                            <div class="space-y-4">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] border-b pb-3 mb-6">Descuentos de Ley</h3>
                                <div class="flex justify-between items-center p-4">
                                    <span class="text-slate-500 font-bold text-sm">Salud (BD)</span>
                                    <span class="font-black text-red-500">-$<?= number_format($liquidacion['salud'], 0, ',', '.') ?></span>
                                </div>
                                <div class="flex justify-between items-center p-4">
                                    <span class="text-slate-500 font-bold text-sm">Pensión (BD)</span>
                        ...
                        <div class="p-8 bg-slate-50 border-t flex gap-4">
                            <button onclick="window.print()" class="px-6 py-4 bg-white border-2 border-slate-200 rounded-2xl font-bold text-slate-600 hover:bg-slate-100 transition"> <i class="fas fa-print"></i> </button>
                            <form action="guardar_nomina.php" method="POST" class="flex-1">
                                ...
                                <button type="submit" class="w-full bg-emerald-500 text-white py-4 rounded-2xl font-black text-lg shadow-xl hover:bg-emerald-600 transition flex items-center justify-center gap-3">
                                    <i class="fas fa-check-double"></i> FINALIZAR Y GUARDAR
                                </button>
                            </form>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="h-[400px] border-4 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center text-slate-300">
                        <i class="fas fa-microchip text-6xl mb-4"></i>
                        <p class="font-black uppercase tracking-widest text-xs">Esperando procesamiento de datos...</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
